...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
        ...
        if (node != null) {
            // Search for value in left subtree
            if (value < node.value) {
                node.leftChild = deleteElement(value, node.leftChild);
            // Search for value in right subtree
            } else if (value > node.value) {
            ...
            // Node with given value has been found
                ...
                // Node has 2 children
                    ...
                    // Find successor (the smallest node in right subtree)
                    Node successor = minValueNode(node.rightChild);
                    // Replace the value of the current node with the value of the successor node
            ...
            if (node != null) {
                // Update height after node has been deleted
                node.height = getMaxHeight(getHeight(node.leftChild), getHeight(node.rightChild)) + 1;
                // Check if tree is balanced
                int balance = getBalanceFactor(node);
                // Tree is left heavy
                if (balance > 1) {
                    // Left heavy left child
                    if (getBalanceFactor(node.leftChild) >= 0) {
                        node = LLRotation(node); // Right rotation
                    // Right heavy left child
                    } else {
                        node.leftChild = RRRotation(node.leftChild); // Left rotation
                        node = LLRotation(node); // Right rotation
                    }
                // Tree is right heavy
                } else if (balance < -1) {
                    // Right heavy right child
                    if (getBalanceFactor(node.rightChild) <= 0) {
                        node = RRRotation(node); // Right rotation
                    // Left heavy right child
                    } else {
                        node.rightChild = LLRotation(node.rightChild); // Right rotation
                        node = RRRotation(node); // Left rotation
                    }
                }
            }
        }
